...
function getOffsetRect(e) {
    var t = e.getBoundingClientRect(), i = document.body, s = document.documentElement, a = window.pageYOffset || s.scrollTop || i.scrollTop, n = window.pageXOffset || s.scrollLeft || i.scrollLeft, r = s.clientTop || i.clientTop || 0, d = s.clientLeft || i.clientLeft || 0, o = t.top + a - r, l = t.left + n - d;
...
window.dhtmlXScheduler = window.scheduler = {version: "4.3.0"}, window.dhtmlx || (dhtmlx = function (e) {
    ...
    var r = [], d = t.evaluate(e, a, function () {
        return i
    }, n, null);
    ...
    setTransactionMode: function (e, t) {
        ...
        if (e) {
            ...
            var i = new dtmlXMLLoaderObject(function (i, s, a, n, r) {
                var d = [];
                if (t)d.push(t); else if (e)for (var o in e)d.push(o);
            ...
            else if ("POST" == this._tMode)i.loadXML(s, !0, this.serialize(e, t)); else if ("REST" == this._tMode) {
                var a = this.getState(t), n = s.replace(/(\&|\?)editing\=true/, "");
        ...
        if (window.JSON)try {
            var s = JSON.parse(t.xmlDoc.responseText), a = s.action || this.getState(i) || "updated", n = s.sid || i[0], r = s.tid || i[0];
        ...
        i = i.replace("editing=true&", ""), this.getUpdates(i, function () {
            ...
            if (i.length) {
                ...
                for (var s = 0; s < i.length; s++) {
                    var a = i[s].getAttribute("status"), n = i[s].getAttribute("id"), r = i[s].getAttribute("parent");
                    switch (a) {
                        case"inserted":
    ...
    dhtmlxEventable(scheduler), scheduler._detachDomEvent = function (e, t, i) {
        ...
        if (t = t || e.target || e.srcElement, !this.config.readonly) {
            ...
            switch (i) {
                ...
                default:
                    ...
                    if (a)a.call(this, e); else if (t.parentNode && t != this)return scheduler._on_dbl_click(e, t.parentNode)
            }
        }
        ...
        if (this._drag_mode) {
            ...
            if (t.force_redraw || this._is_pos_changed(this._drag_pos, t)) {
                ...
                if (this._edit_id != this._drag_id && this._close_not_saved(), this._drag_pos = t, this._drag_pos.has_moved = !0, "create" == this._drag_mode) {
                    ...
                    var n = new Date(this._drag_start), r = new Date(s);
                ...
                if (scheduler.config.limit_drag_out && (+u < +scheduler._min_date || +s > +scheduler._max_date)) {
                    var g = s - u;
                    +u < +scheduler._min_date ? (u = new Date(scheduler._min_date), s = new Date(+u + g)) : (s = new Date(scheduler._max_date), u = new Date(+s - g));
                    var c = new Date(s - 1)
                }
                if (!this._table_view && !scheduler.config.all_timed && (!scheduler._get_section_view() && t.x != this._get_event_sday({
                    ...
                    var g = s - u, h = this._min_date.valueOf() + 24 * t.x * 60 * 6e4;
                ...
                if (this._table_view || c.getDate() == u.getDate() && c.getHours() < this.config.last_hour || scheduler._allow_dnd)if (o.start_date = u, o.end_date = new Date(s), this.config.update_render) {
                    var v = scheduler._els.dhx_cal_data[0].scrollTop;
                    this.update_view(), scheduler._els.dhx_cal_data[0].scrollTop = v
            ...
            var f = this._locate_event(e.target || e.srcElement);
            this.callEvent("onMouseMove", [f, e])
        }
        ...
        if (!e || 2 != e.button || !scheduler.config.touch) {
            if (this._drag_mode && this._drag_id) {
                ...
                var t = this._drag_id, i = this._drag_mode, s = this.getEvent(this._drag_id);
                if (this._drag_event._dhx_changed || !this._drag_event.start_date || s.start_date.valueOf() != this._drag_event.start_date.valueOf() || s.end_date.valueOf() != this._drag_event.end_date.valueOf()) {
                    ...
                    } else this._new_event || this.callEvent(a ? "onEventAdded" : "onEventChanged", [t, this.getEvent(t)]); else a ? this.deleteEvent(s.id, !0) : (this._drag_event._dhx_changed = !1, this._clear_temporary_properties(s, this._drag_event), scheduler._lame_copy(s, this._drag_event), this.updateEvent(s.id))
                }
        ...
        var s = e, a = this._get_columns_num(t, i);
        ...
        var i = new Date(scheduler._min_date), s = new Date(scheduler._max_date), a = this.date.date_part(scheduler._currentDate()), n = parseInt(e.style.width, 10), r = new Date(this._min_date), d = this._get_columns_num(i, s);
        ...
        if (n)for (var r = new Date(e), d = 0; t > d; d++)n(r) && (this._ignores_detected += 1, this._ignores[d] = !0, a && t++), r = scheduler.date.add(r, s, i), scheduler.date[i + "_start"] && (r = scheduler.date[i + "_start"](r))
        ...
        var a = scheduler.date.add(t, 1, "month"), n = new Date(i), r = scheduler._currentDate();
        ...
        for (var _ = 0, c = "<table cellpadding='0' cellspacing='0'>", u = [], l = 0; d > l; l++) {
            c += "<tr>";
            for (var g = Math.max(s(l) - scheduler.xy.month_head_height, 0), v = 0; 7 > v; v++) {
                c += "<td";
                var f = "";
                t > i ? f = "dhx_before" : i >= a ? f = "dhx_after" : i.valueOf() == r.valueOf() && (f = "dhx_now"), this._ignores_detected && this._ignores[v] && (f += " dhx_scale_ignore"), c += " class='" + f + " " + this.templates.month_date_class(i, r) + "' >";
                var m = "dhx_month_body", p = "dhx_month_head";
                0 === v && this.config.left_border && (m += " dhx_month_body_border", p += " dhx_month_head_border"), this._ignores_detected && this._ignores[v] ? c += "<div></div><div></div>" : (c += "<div class='" + p + "'>" + this.templates.month_day(i) + "</div>", c += "<div class='" + m + "' " + o[v] + ";height:" + g + "px;'></div></td>"), u.push(i);
                var x = i.getDate();
        ...
        var n = Math.ceil(Math.round((s.valueOf() - i.valueOf()) / 864e5) / 7), r = Math.floor(e.clientHeight / n) - this.xy.month_head_height;
        ...
        for (var i = this.config.lightbox.sections, s = 0; s < i.length; s++)if (i[s].map_to == e)for (var a = i[s].options, n = 0; n < a.length; n++)if (a[n].key == t)return a[n].label;
        return ""
        ...
        if (i._start_correction || i._end_correction) {
            ...
            var d = 60 * (i.last_hour - i.first_hour), o = Math.ceil((t / 6e4 - r) / d);
            t += o * (1440 - d) * 60 * 1e3
        }
        ...
        init: function () {
            ...
            var s = i.getFullYear(), a = Math.round((i.getTime() - new Date(s, 0, 1).getTime()) / 864e5), n = 1 + Math.floor(a / 7);
            return n
        ...
        date: {
            ...
            month_short: ["Янв", "Февр", "Март", "Апр", "Май", "Июнь", "Июль", "Авг", "Сент", "Окт", "Нояб", "Дек"],
            day_full: ["Воскресенье", "Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота", "Воскресенье"],
        ...
        labels: {
            ...
            month_tab: "Месяц",
            new_event: "New event",
            icon_save: "Сохранить бронирование",
            icon_cancel: "&#215;",
            icon_details: "Details",
            icon_edit: "Edit",
            icon_delete: "Удалить бронь",
            confirm_closing: "",
            confirm_deleting: "Вы точно хотите удалить бронь?",
        ...
        if (t) {
            ...
            if (i)return !0;
            var s = (e.end_date.valueOf() - e.start_date.valueOf()) / 36e5, a = 24 - (this.config.last_hour - this.config.first_hour);
            return s > a
        }
        return !1
        ...
        for (var t = this._rendered_location, i = this._lame_clone(e), s = e._sday; s < e._eday; s++) {
            ...
            var a = scheduler.date, n = scheduler._min_date;
            n = a.add(n, i._sweek, "week"), n = a.add(n, i._sday, "day");
            var r = scheduler.getEvents(n, a.add(n, 1, "day")).length, d = this._get_event_bar_pos(i), o = d.x2 - d.x, l = document.createElement("div");
        ...
        var i = this.xy.bar_height, s = this._colsS.heights, a = this._colsS.heights = [0, 0, 0, 0, 0, 0, 0], n = this._els.dhx_cal_data[0];
        if (e = this._table_view ? this._pre_render_events_table(e, t) : this._pre_render_events_line(e, t), this._table_view)if (t)this._colsS.heights = s; else {
            ...
            if (r.rows) {
                for (var d = 0; d < r.rows.length; d++) {
                    a[d]++;
                    var o = r.rows[d].cells, l = this._colsS.height - this.xy.month_head_height;
                    if (a[d] * i > l) {
                        var h = l;
                ...
                var v = (r.parentNode.childNodes, (a[0] + 1) * i + 1), f = v, m = v + "px";
                this.config.multi_day_height_limit && (f = Math.min(v, this.config.multi_day_height_limit), m = f + "px"), n.style.top = this._els.dhx_cal_navline[0].offsetHeight + this._els.dhx_cal_header[0].offsetHeight + f + "px", n.style.height = this._obj.offsetHeight - parseInt(n.style.top, 10) - (this.xy.margin_top || 0) + "px";
                ...
                if (i[n._sday] || (i[n._sday] = []), !t) {
                    ...
                    for (var u = h.length, g = !1, v = 0; v < h.length; v++) {
                        ...
                        if (c.valueOf() <= n.start_date.valueOf()) {
                            g = !0, n._sorder = _._sorder, u = v, n._inner = !0;
                            break
                        }
                    }
        ...
        for (var i, s = [], a = [[], [], [], [], [], [], []], n = this._colsS.heights, r = this._cols.length, d = {}, o = 0; o < e.length; o++) {
            ...
            if (l._sday = g % r, !this._ignores[l._sday] || !l._timed) {
                var v = this.locate_holder_day(u, !0, l) || r;
                l._eday = v % r || r, l._length = v - g, l._sweek = Math.floor((this._correct_shift(c.valueOf(), 1) - this._min_date.valueOf()) / (864e5 * r));
                var f, m = a[l._sweek];
                for (f = 0; f < m.length && !(m[f]._eday <= l._sday); f++);
                if (l._sorder && t || (l._sorder = f), l._sday + l._length <= r)i = null, s.push(l), m[f] = l, n[l._sweek] = m.length - 1, l._first_chunk = _.first_chunk, l._last_chunk = _.last_chunk; else {
                    var p = this._copy_event(l);
                    p.id = l.id, p._length = r - l._sday, p._eday = r, p._sday = l._sday, p._sweek = l._sweek, p._sorder = l._sorder, p.end_date = this.date.add(c, p._length, "day"), p._first_chunk = _.first_chunk, _.first_chunk && (_.first_chunk = !1), s.push(p), m[f] = p, i = p.end_date, n[l._sweek] = m.length - 1, o--
                }
            }
        }
        return s
        ...
        t = t || 0;
        var i = 60 * e.start_date.getHours() + e.start_date.getMinutes(), s = 60 * e.end_date.getHours() + e.end_date.getMinutes() || 60 * scheduler.config.last_hour, a = this._y_from_date(e.start_date), n = Math.max(t, (s - i) * this.config.hour_size_px / 60);
        return {top: a, height: n}
        ...
        if (!(e._sday < 0)) {
            ...
            if (s) {
                var a = this._calc_event_y(e, scheduler.xy.min_event_height), n = a.top, r = a.height, d = e._count || 1, o = e._sorder || 0, l = Math.floor((s.clientWidth - i) / d), h = o * l + 1;
                ...
                if (this._rendered.push(g), s.appendChild(g), h = h + parseInt(s.style.left, 10) + i, this._edit_id == e.id) {
                    ...
                    this.set_xy(f, l - 6, r - 26), f.style.cssText += ";margin:2px 2px 2px 2px;overflow:hidden;", g.appendChild(f), this._els.dhx_cal_data[0].appendChild(g), this._rendered.push(g), f.innerHTML = "<textarea class='dhx_cal_editor'>" + e.text + "</textarea>", this._quirks7 && (f.firstChild.style.height = r - 12 + "px"), this._editor = f.firstChild, this._editor.onkeydown = function (e) {
        ...
        c && (_ = _ + " " + c);
        var u = e.color ? "background:" + e.color + ";" : "", g = e.textColor ? "color:" + e.textColor + ";" : "", v = '<div event_id="' + h + '" class="' + _ + '" style="position:absolute; top:' + i + "px; left:" + t + "px; width:" + (s - 4) + "px; height:" + a + "px;" + (n || "") + '"></div>';
        l.innerHTML = v;
        var f = l.cloneNode(!0).firstChild;
        if (!o && scheduler.renderEvent(f, e, s, a, r, d))return f;
        f = l.firstChild;
        ...
        return o && (p = "dhx_resize_denied " + p), m += '<div class="' + p + '" style=" width:' + (s - 8) + "px;" + (o ? " margin-top:-1px;" : "") + u + g + '" ></div>', f.innerHTML = m, f
        ...
        var s = this.xy.bar_height, a = e._sorder;
        ...
        var r = a * s, d = this._colsS.heights[e._sweek] + (this._colsS.height ? this.xy.month_scale_height + 2 : 2) + r;
        return {x: t, x2: i, y: d}
    },scheduler.render_event_bar = function (e) {
        var t = this._rendered_location, i = this._get_event_bar_pos(e), s = i.y, a = i.x, n = i.x2, r = "";
        if (n) {
            ...
            g && (u += " " + g);
            var v = e.color ? "background:" + e.color + ";" : "", f = e.textColor ? "color:" + e.textColor + ";" : "", m = ["position:absolute", "top:" + s + "px", "left:" + a + "px", "width:" + (n - a - 15) + "px", f, v, e._text_style || ""].join(";"), p = '<div event_id="' + e.id + '" class="' + u + '" style="' + m + '">';
        ...
        for (var v = e.doXPath("//userdata"), n = 0; n < v.length; n++) {
            var f = this._xmlNodeToJSON(v[n]);
        ...
        parse: function (e) {
            ...
            var i = e.indexOf(":");
            if (-1 != i) {
                var s = e.substr(0, i).toLowerCase(), a = e.substr(i + 1).replace(/\\\,/g, ",").replace(/[\r\n]+$/, "");
            ...
            var a = s[0].substr(0, 4), n = parseInt(s[0].substr(4, 2), 10) - 1, r = s[0].substr(6, 2);
        ...
        var a = document.getElementById(s.id), n = a.nextSibling, r = {
            section: s, header: a, node: n, getValue: function (e) {
        ...
        template: {
            ...
            render: function (e) {
                ...
                var t = e.time_format, i = scheduler.config, s = this.date.date_part(scheduler._currentDate()), a = 1440, n = 0;
                ...
                var s = e.getElementsByTagName("select"), a = i._time_format_order;
        ...
        for (var a = this.config.lightbox.sections, n = 0; n < a.length; n++) {
            var r = a[n], d = document.getElementById(r.id).nextSibling, o = this.form_blocks[r.type], l = void 0 !== i[r.map_to] ? i[r.map_to] : r.default_value;
        ...
        for (var t = this.config.lightbox.sections, i = 0; i < t.length; i++) {
            ...
            s = s ? s.nextSibling : s;
            var a = this.form_blocks[t[i].type], n = a.get_value.call(this, s, e, t[i]);
            ...
            if (t.className || (t = t.previousSibling), t && t.className)switch (t.className) {
                ...
                default:
                    if (t.getAttribute("dhx_button"))scheduler.callEvent("onLightboxButton", [t.className, t, e]); else {
                        var s, a, n;
                        -1 != t.className.indexOf("dhx_custom_button") && (-1 != t.className.indexOf("dhx_custom_button_") ? (s = t.parentNode.getAttribute("index"), n = t.parentNode.parentNode) : (s = t.getAttribute("index"), n = t.parentNode, t = t.firstChild)), s && (a = scheduler.form_blocks[scheduler.config.lightbox.sections[s].type], a.button_click(s, t, n, n.nextSibling))
                    }
            }
        ...
        function a(e, t, i) {
            if (e && t) {
                var s = Math.abs(e.pageY - t.pageY), a = Math.abs(e.pageX - t.pageX);
                a > i && (!s || a / s > 3) && (e.pageX > t.pageX ? scheduler._click.dhx_cal_next_button() : scheduler._click.dhx_cal_prev_button())
            }
        }

        function n(e) {
            var t = scheduler.getState().drag_mode, i = scheduler.matrix ? scheduler.matrix[scheduler._mode] : !1, s = scheduler.render_view_data;
        ...
        var d, o, l, h, _, c, u = -1 != navigator.userAgent.indexOf("Android") && -1 != navigator.userAgent.indexOf("WebKit"), g = 0;
            ...
            a && (a.padding = 14), scheduler.templates.event_bar_date = function (e) {
                ...
                if (scheduler.templates.month_day = function (i) {
                    ...
                    for (var i = scheduler._els.dhx_cal_navline[0].getElementsByTagName("div"), s = null, a = 211, n = 0; n < i.length; n++) {
                        var r = i[n], d = r.getAttribute("name");
            ...
            if ("string" != typeof s) {
                ...
                return this.each(function () {
                    if (this && this.getAttribute)if (this.getAttribute("dhxscheduler"))a.push(window[this.getAttribute("dhxscheduler")]);
                        ...
                        this.getElementsByTagName("div").length || (this.innerHTML = '' +
                        '<div class="dhx_cal_navline">' +
                            ...
                            '</div><div class="dhx_cal_tab" name="week_tab" style="right:140px;"></div>' +
                            '<div class="dhx_cal_tab" name="month_tab" style="right:76px;"></div>' +
                        '</div>' +
                        '<div class="dhx_cal_header"></div>' +
                        '<div class="dhx_cal_data" style="height: 100%;"></div>', this.className += " dhx_cal_container"), i.init(this, i.config.date, i.config.mode), s.data && i.parse(s.data), a.push(i), t++
                    }
                }), 1 === a.length ? a[0] : a
            }
        ...
        var t = scheduler.setCurrentView, i = scheduler.updateView, s = null, a = null, n = function (t, n) {
            var r = this;
    ...
    var i = i ||
        ...
        '<div class="dhx_cal_tab" name="month_tab" style="right:76px;"></div>', a = document.createElement("DIV");
    return a.id = "dhxSchedObj_" + this._genStr(12), a.innerHTML = '<div id="' + a.id + '" class="dhx_cal_container" style="width:100%; height:100%;">' +
    '<div class="dhx_cal_navline">' +
        '<div class="dhx_cal_today_button"></div>' +
        '<div class="dhx_cal_date">' + '<div class="dhx_cal_prev_button">&nbsp;</div>' +
    '<div class="dhx_cal_next_button">&nbsp;</div>' + '</div>' + i + '' +
    '</div>' +
    '<div class="dhx_cal_header"></div>' +
    '<div class="dhx_cal_data"  style="height: 100%;"></div>' +
    '</div>', document.body.appendChild(a.firstChild), this.attachObject(a.id, !1, !0), this.vs[this.av].sched = s, this.vs[this.av].schedId = a.id, s.setSizes = s.updateView, s.destructor = function () {
    }, s.init(a.id, e, t), this.vs[this._viewRestore()].sched
});
